...
const AGENTIC_TOOLS_REPO_URL = "https://github.com/swiftpostlab/agentic-tools";
/**
 * @param {unknown} value
 */
 ...
 * @returns {string}
 */
const buildSyncedSkillsGitignore = (skillNames) => {
    const lines = [
 ...
 * @returns {string[]}
 */
const writeSyncedSkillsGitignore = (destinationSkillsDir, skillNames, dryRun) => {
    const gitignorePath = path.join(destinationSkillsDir, GITIGNORE_FILENAME);
    const content = buildSyncedSkillsGitignore(skillNames);
...
const handleSyncCommand = (parsed, options) => {
    ...
    for (const message of writeSyncedSkillsGitignore(destinationSkillsDir, linkedSkillNames, parsed.dryRun)) {
        options.output(message);
    }
    return 0;
};
