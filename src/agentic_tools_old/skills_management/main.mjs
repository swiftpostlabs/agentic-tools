...
const AGENTIC_TOOLS_REPO_URL = "https://github.com/swiftpostlabs/agentic-tools";
/**
 * @param {unknown} value
 */
...
/**
 * @param {Iterable<string>} skillNames
 * @param {string[]} [sourceUrls]
 * @returns {string}
 */
const buildSyncedSkillsGitignore = (skillNames, sourceUrls = []) => {
    const lines = [
        "# Generated by agentic-tools `skills sync` — do not edit by hand.",
        `# ${AGENTIC_TOOLS_REPO_URL}`,
    ];
    const orderedUrls = deduplicatePreservingOrder(sourceUrls);
    if (orderedUrls.length > 0) {
        lines.push("# Skills synced from:");
        lines.push(...orderedUrls.map((url) => `#   ${url}`));
    }
    lines.push("# Ignores skill symlinks synced into this directory.");
    lines.push(...[...skillNames].sort());
    return `${lines.join("\n")}\n`;
};
/**
 * @param {string} destinationSkillsDir
 ...
 * @returns {string[]}
 */
const writeSyncedSkillsGitignore = (destinationSkillsDir, skillNames, sourceUrls, dryRun) => {
    const gitignorePath = path.join(destinationSkillsDir, GITIGNORE_FILENAME);
    const content = buildSyncedSkillsGitignore(skillNames, sourceUrls);
    const existing = isFile(gitignorePath)
...
const handleSyncCommand = (parsed, options) => {
    ...
    for (const configuredSource of loadConfiguredSkillSources(configPath)) {
        ...
        if (configuredSource.url !== null) {
            sourceUrls.push(configuredSource.url);
        }
