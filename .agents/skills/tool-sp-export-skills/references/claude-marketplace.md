...
Use this mode when the skills should be **published** — packaged as a Claude Code plugin and
distributed through a `marketplace.json` catalog, so anyone who adds the marketplace can install them
and receive updates. This is the only export mode that is an ongoing distribution channel rather than a
one-time handoff.
...
install-cache rules, hosting for public/private/offline, and the release and update flow — are owned by
**`ref-sp-agents-claude-marketplaces`**. Read that skill and follow it. Do not restate or re-derive its
rules here; they change upstream.
...
- **The publish set is enumerated, and that enumeration *is* the visibility filter.** Claude has no
  concept of skill-level visibility, so it ships whatever the manifest lists. Generate the list from
...
Follow the validation steps in `ref-sp-agents-claude-marketplaces`. At minimum, after a real install
