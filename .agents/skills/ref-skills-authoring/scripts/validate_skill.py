...
def validate_name(skill_dir: Path, frontmatter: dict[str, object], findings: list[Finding]) -> str:
    raw_name = frontmatter.get("name")
    skill_label = skill_dir.name
    if not isinstance(raw_name, str) or not raw_name.strip():
        add(findings, skill_label, "error", "frontmatter must include non-empty string 'name'")
        return skill_label

    name = raw_name.strip()
    skill_label = name
    if not NAME_PATTERN.fullmatch(name):
        add(findings, skill_label, "error", "name must be 1-64 chars of lowercase letters, digits, and hyphens")
    if name.startswith("-") or name.endswith("-") or "--" in name:
        add(findings, skill_label, "error", "name cannot start or end with a hyphen or contain consecutive hyphens")
    if name != skill_dir.name:
        add(findings, skill_label, "error", f"name must match folder name '{skill_dir.name}'")
    if not (name.startswith("ref-") or name.startswith("tool-")):
        add(findings, skill_label, "warning", "repo convention expects skill names to start with ref- or tool-")
    return skill_label


def validate_frontmatter(skill_dir: Path, frontmatter: dict[str, object], findings: list[Finding]) -> str:
    skill_label = validate_name(skill_dir, frontmatter, findings)

    unexpected = sorted(set(frontmatter) - ALLOWED_TOP_LEVEL_KEYS)
    if unexpected:
        add(findings, skill_label, "warning", "unexpected frontmatter keys: " + ", ".join(unexpected))

    description = frontmatter.get("description")
    if not isinstance(description, str) or not description.strip():
        add(findings, skill_label, "error", "frontmatter must include non-empty string 'description'")
    elif len(description) > 1024:
        add(findings, skill_label, "error", "description must be 1024 characters or less")
    elif "<" in description or ">" in description:
        add(findings, skill_label, "error", "description must not contain angle brackets")

    metadata = frontmatter.get("metadata")
    if metadata is None:
        add(findings, skill_label, "warning", "metadata.agentic-tools-category is missing")
    elif not isinstance(metadata, dict):
        add(findings, skill_label, "error", "metadata must be a string-to-string mapping")
    else:
        metadata_mapping = cast(dict[object, object], metadata)
        for key, value in metadata_mapping.items():
            if not isinstance(key, str) or not isinstance(value, str):
                add(findings, skill_label, "error", "metadata keys and values must be strings")
        ...
        if visibility is not None and visibility not in {"shareable", "repo-local"}:
            add(findings, skill_label, "error", "shareable-skills.visibility must be 'shareable' or 'repo-local'")
...
def validate_body(skill_dir: Path, skill_label: str, body: str, findings: list[Finding]) -> None:
    lowered = body.lower()
    if "## purpose" not in lowered:
        add(findings, skill_label, "warning", "SKILL.md should include a Purpose section")
    if "## when to use" not in lowered:
        add(findings, skill_label, "warning", "SKILL.md should include a When to use section")
    if len(body.splitlines()) > 500:
        add(findings, skill_label, "warning", "SKILL.md is over 500 lines; consider progressive disclosure")

    for match in INTERNAL_REFERENCE_PATTERN.finditer(body):
        raw_path = match.group("path").rstrip(".,;:")
        target = skill_dir / raw_path[2:]
        if not target.exists():
            add(findings, skill_label, "error", f"referenced support file does not exist: {raw_path}")

    if "../" in body:
        add(findings, skill_label, "warning", "body contains '../'; prefer same-skill relative or repo-root-relative paths")


def validate_resources(skill_dir: Path, skill_label: str, findings: list[Finding]) -> None:
    for child in sorted(skill_dir.iterdir()):
        if child.name == "SKILL.md" or child.name.startswith("."):
            continue
        if child.is_dir() and child.name not in VALID_RESOURCE_DIRS:
            add(findings, skill_label, "warning", f"unrecognized resource directory '{child.name}'")
...
def validate_skill(skill_dir: Path) -> list[Finding]:
    ...
    if not skill_md.is_file():
        return [Finding(skill=skill_dir.name, severity="error", message="SKILL.md not found")]
...
def find_skill_dirs(target: Path, validate_all: bool) -> list[Path]:
    if validate_all:
        return sorted(child for child in target.iterdir() if child.is_dir() and (child / "SKILL.md").is_file())
    return [target]
...
def main() -> int:
    ...
    parser.add_argument("target", type=Path, help="Skill directory, or skills root with --all")
    parser.add_argument("--all", action="store_true", help="Validate every child skill directory")
    ...
    if args.format == "json":
        print(json.dumps({"checked": len(skill_dirs), "findings": [asdict(finding) for finding in findings]}, indent=2))
    else:
