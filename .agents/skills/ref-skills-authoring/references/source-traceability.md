...
Use this file when you need to verify where a `ref-skills-authoring` rule came from.

## Purpose

This document maps important guidance in the `ref-skills-authoring` skill to its source.
...
| `ref-skills-authoring` includes provider-agnostic helper scripts for skill validation and eval aggregation. | Local | The Claude skill-creator bundle includes useful validation and aggregation tools, but its trigger optimizer and execution harness depend on Claude-specific command behavior. This repo keeps only portable deterministic scripts under `ref-skills-authoring/scripts/`. | [Using scripts in skills](https://agentskills.io/skill-creation/using-scripts), [Specification](https://agentskills.io/specification), local `.agents/tasks/claude-skills-creator/skill-creator/` bundle (Apache-2.0) | [Tool And Script Rules](../SKILL.md#tool-and-script-rules) | 2026-05-21 |
...
When `ref-skills-authoring` changes, update this file if the change:
