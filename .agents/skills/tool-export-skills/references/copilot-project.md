...
1. Check whether the target repo can use `.agents/skills.json` plus `uv run skills-management sync`.
...
For Copilot projects, prefer `skills-management` when the target repo can consume shared skills directly. If the user really wants only a one-off conversation, use the AI-conversation mode instead.