...
1. Check whether the target repo can use `.agents/config.json` plus `uv run agentic-tools skills sync`.
...
- the recommended `.agents/config.json` block or the fallback manual-bundle plan
