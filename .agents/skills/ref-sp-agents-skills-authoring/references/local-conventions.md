...
Use this file when you need to know which `ref-sp-agents-skills-authoring` rules are intentional repo conventions rather than part of the official Agent Skills references.
...
| Keep traceability and conventions docs under `ref-sp-agents-skills-authoring/references/` rather than the entry `SKILL.md`. | This preserves progressive disclosure and keeps the entry skill focused on operational guidance. | Consistent with the original progressive-disclosure model. | Revisit only if the package becomes too fragmented. | 2026-05-01 |
...
- Keep traceability and local-conventions docs in `ref-sp-agents-skills-authoring/references/` instead of the entry `SKILL.md`.
...
Before adding a new convention to `ref-sp-agents-skills-authoring`, ask:
