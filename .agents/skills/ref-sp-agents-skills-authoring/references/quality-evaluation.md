...
Use this table when reviewing why only part of the local Claude skill-creator bundle was promoted into `ref-sp-agents-skills-authoring`.

| Source artifact | Decision | Reason |
| --- | --- | --- |
| `scripts/quick_validate.py` | Adapted as `./scripts/validate-skill.mts` (TypeScript, Node >= 22). | The validation idea is portable, but the implementation was rewritten to avoid a YAML dependency, support catalog-wide checks, respect this repo's metadata conventions, and report warnings/errors predictably. Sharing-spec checks were split out to `ref-sp-agents-shareable-skills/scripts/validate-sharing.mts`. |
...
- For sharing-spec conformance, also run `node ../ref-sp-agents-shareable-skills/scripts/validate-sharing.mts .agents/skills --all`.
