...
metadata:
  ...
  reason: "This reference documents this repo's concrete skill-category taxonomy and catalog conventions."
---
...
| --- | --- | --- |
| `agents` | Agent behavior, instructions, security, local task tracking, skills authoring, skills sharing, policy generation, and repo-specific agent tooling. | `ref-sp-agents-persona`, `ref-sp-agents-skills-authoring`, `ref-sp-agents-policy`, `tool-sp-maintain-skills` |
| `app` | Whole-app guidance and application-level stack decisions. | `ref-sp-js-web-standalone`, `ref-sp-js-react-next` |
| `db` | Database modeling, relational and non-relational design, normalization, transactions, and database operations guidance. | `ref-sp-db-management`, `ref-sp-db-relational`, `ref-sp-db-normalization`, `ref-sp-db-nosql` |
| `dev` | Generic development workflows that are not language-specific, such as coding defaults, commits, versioning, and package-management policy. | `ref-sp-dev-coding-patterns`, `ref-sp-dev-semantic-versioning`, `ref-sp-dev-git-commits`, `tool-sp-commit` |
| `docs` | Documentation authoring and README structure. | `ref-sp-dev-docs-authoring` |
| `github` | GitHub platform automation and repository services. | `ref-sp-dev-github-actions-ci`, `ref-sp-dev-github-dependabot` |
| `js` | JavaScript, TypeScript, Deno, React, Next.js, userscripts, and browser-script domains. | `ref-sp-js-typescript`, `ref-sp-js-deno`, `ref-sp-js-react` |
| `project` | Project layout, architecture boundaries, feature folders, and repository setup. | `ref-sp-dev-project-setup`, `ref-sp-dev-projects-architecture` |
| `py` | Python-specific typing, CLIs, tests, tooling, and this repo's Python conventions. | `ref-sp-py-python`, `ref-sp-py-commitizen`, `ref-sp-py-code-conventions` |
| `supabase` | Supabase platform behavior, CLI workflows, migrations, API use, and Edge Functions. | `ref-sp-platform-supabase` |

## Decision Rules

- Choose the domain of the guidance, not the organization-specific prefix. For example, `ref-sp-agents-policy` is category `agents`, not `swiftpost`.
- Choose the domain of the action, not the `tool-...` prefix. For example, `tool-sp-commit` is category `dev`, while `tool-sp-handle-agents-local-tasks` is category `agents`.
...
| `ref-sp-agents-skills-management` | `agents` |
| `ref-sp-agents-shareable-skills`, `ref-sp-agents-skills-authoring` | `agents` |
| `tool-*skills*`, `tool-*agents*`, `tool-sp-adopt-these-skills` | `agents` |
| `ref-app-*` | `app` |
| `ref-db-*` | `db` |
| `ref-sp-dev-coding-patterns`, `ref-dev-*`, `ref-sp-dev-git-commits`, `tool-sp-commit` | `dev` |
| `ref-sp-dev-docs-authoring` | `docs` |
| `ref-github-*` | `github` |
| `ref-js-*` | `js` |
| `ref-project-*`, `ref-projects-*` | `project` |
| `ref-sp-py-python`, `ref-py-*`, `ref-sp-py-code-conventions` | `py` |
| `ref-sp-platform-supabase` | `supabase` |
